...
|  |  |  |  |  |
|---|---|---|---|---|
| 🟢 **Live on Coston2** | ✅ **195 tests / 20 suites** | ✅ **8 invariants** | ✅ **4 Flare primitives (all load-bearing)** | ✅ **Source verified** |

**Quick links:** [Latest Vault on Coston2 Explorer](https://coston2-explorer.flare.network/address/0x19190FF7261568C9bf4bbA7385604df804f8a01c) · [🎛️ Live Demo](https://act-voters-exhibition-sauce.trycloudflare.com) · [Demo script](#demo-script-for-judges) · [Deployment](#latest-coston2-deployment-2026-08-10) · [Test suite](#test-suite--191-tests--19-suites)
...
Flare's FAssets launch has triggered explosive XRPFi growth — **$86.2M worth of XRP** already bridged to Flare as FXRP (The Defiant, Sep 2025), TVL surged **160% to $120M+** after USDT0 rollout (The Block), and FXRP was just approved as collateral in Sentora's **$280M institutional vault** on Ethereum (Aug 2026). Yet this growing pool of productive collateral has **zero native credit market** against it. XRP holders won't sell their position, and can't borrow against it without granting a centralized credit bureau visibility into their finances — which they won't grant. Every existing lending protocol demands an all-or-nothing privacy tradeoff: reveal everything, or borrow nothing.
...
7. **8 stateful invariant properties** — configured at 64 runs × 96 calls, with a seeded funded loan and lifecycle actions
8. **Latest live Coston2 deployment** — vault at `0x19190FF7261568C9bf4bbA7385604df804f8a01c`, child CreditScoreSBT at `0xc5b44930EAD19D5B9284A05a356e2758152C549c`, deployment tx `0x94cc33a41ecf97164d6db33724a630012f281c3869e91a6316fa7f26216651b1` (USDT0 liquidity must be supplied by a lender before borrowing)
...
13. **195-test Foundry suite across 20 suites + 33 Python TEE handler tests** — grew from 91 → 191 during the program
...
|-------|------|----------------------------------------|
| `/` | Landing | One-screen project pitch: title, one-liner, bounty badges, the 7-step flow, key numbers (195 tests, 4 primitives, live on Coston2). Connect-wallet CTA. The 5-minute skim. |
...
| `/docs/testing` | Testing | Full test suite breakdown: all 20 suites, 195 tests, coverage %, per-suite one-liner, reproduce commands. Mirrors [Test Suite](#test-suite--191-tests--19-suites). |
...
| **5. Authority registration** | Vault constructor sets the TEE authority (immutable) — attestations from this enclave only | documented in deploy section |
...
## Test Suite — 195 Tests · 20 Suites

**195/195 passed · 0 failures · 0 skipped · 97.75% line coverage of `CreditGateVault.sol` · 97.87% function coverage**

Verified live with `forge test` → `Ran 20 test suites ... 195 tests passed, 0 failed, 0 skipped`.
...
| 16 | `CreditGateVaultRegistryTest` | `test/CreditGateVault.registry.t.sol` | **4** | ContractRegistry integration: `updateFdcVerificationFromRegistry` / `updateFtsoV2FromRegistry` owner-only access control, rejects zero address, updates verification addresses (future-proofs against protocol upgrades) |
...
forge test                     # 195 tests, 20 suites, 0 failures
forge test --summary           # per-suite breakdown
forge coverage                 # 96.96% lines / 97.87% functions on CreditGateVault.sol
...
All findings from the internal security review (`planning/security-audit/verdict.md` = **PASS-WITH-NOTES**) were remediated. No Critical issues. Final test status: **195/195 passing, 20 suites, 0 failures**.
...
All commits are on `main` and reproduce to the 195/195 passing sweep.
...
| **FAssets/FXRP** | ✅ Load-bearing | ✅ | ❌ | ❌ | ✅ (MockFXRP in vault) | ❌ |
| **FTSOv2** | ✅ Load-bearing | ❌ | ✅ | ❌ | ✅ (price-drift) | ❌ |
| **FCC/TEE** | ✅ Load-bearing (Go + Python) | ✅ | ✅ | ⚠️ NOX/SGX, NOT Flare FCC | ✅ (vTPM) | ✅ (FCC only) |
| **FDC** | ✅ Load-bearing | ✅ | ❌ | ❌ | ✅ (Payment attestation) | ❌ |
| **# Flare primitives (load-bearing)** | **4 of 4** | 2 of 4 | 3 of 4 | 0 of 4 | 4 of 4 | 1 of 4 |
| **Tests** | **195 / 20 suites / 0 failures / 97.75% coverage** | No test claims | No test claims | Not surfaced | 6 tests | No test claims |
...
1. **One of only two submissions using all 4 Flare primitives as load-bearing** — FAssets/FXRP + FTSOv2 + FCC + FDC. AegisFlow omits FTSO; FlareShield AI omits FDC; Axi uses a non-Flare TEE; VeriFlow AI is FCC-only. Whisper also uses all 4 (FXRP vault + FTSO drift check + vTPM attestation + FDC payment proof) but with a fundamentally different use case (dark pool trading, not credit). CreditGate is the only submission that binds **FCC (private credit eligibility) → FDC (public cross-chain repayment verification)** in a single product flow — the credit lifecycle is the product, not a side effect.
2. **Only submission binding FCC → FDC for credit lifecycle** — TEE-verified credit eligibility → FDC-verified XRPL repayment. Whisper uses FDC for settlement (not credit), and AegisFlow uses FDC for sanctions screening (not repayment). CreditGate is the only one where FDC verifies the credit lifecycle itself.
...
4. **Test depth** — 195 tests / 20 suites / 0 failures / 97.75% line coverage. Whisper has 6 tests; the others publish zero test claims. The deepest **verifiable** engineering evidence in the field, reconstructible by any judge via `forge test`.
...
forge test   # → "195 tests passed, 0 failures" across 20 suites
```
...
## Latest Coston2 Deployment (2026-08-10)

The latest source-compatible deployment is live and verified on chain:

- **CreditGateVault:** `0x19190FF7261568C9bf4bbA7385604df804f8a01c`
- **CreditScoreSBT:** `0xc5b44930EAD19D5B9284A05a356e2758152C549c`
- **Deploy transaction:** `0x94cc33a41ecf97164d6db33724a630012f281c3869e91a6316fa7f26216651b1`
- **Owner / TEE authority:** `0x5a3969F3767Cde96D662A94cAa79779073F80A0c`
- **Explorer:** [Vault](https://coston2-explorer.flare.network/address/0x19190FF7261568C9bf4bbA7385604df804f8a01c)

The deployment includes the current vault implementation and child SBT. The vault currently has **0 USDT0 liquidity** because the deployment wallet has no USDT0 balance; the frontend therefore exposes the live protocol and SBT, while borrowing requires a lender transfer first.
...
```bash
# 1. Contracts — 195 tests across 20 suites, 0 failures
forge test
...
git clone --recurse-submodules https://github.com/tommycet/creditgate.git creditgate
cd creditgate
...
# 4. Run the Solidity test suite (195 tests across 20 suites)
forge test
...
├── test/                      # 195 tests across 20 suites (see Test Suite above)
├── script/
...
### Hackathon scope ✅
Two FCC paths shipped (Go handler for local testnet + Python TEE handler for GCP Confidential Space via Intel TDX), pre-captured FDC proof demo, deployed to Coston2 with 191 Foundry tests + 33 Python tests.

### Post-hackathon (Q4 2026)

| Phase | Milestone | Timeline | Success metric |
|-------|-----------|----------|----------------|
| **1. Security audit** | External firm audit of CreditGateVault.sol + CreditScoreSBT.sol | 4–6 weeks | 0 Critical findings, publish report |
| **2. Mainnet deployment** | Deploy to Flare mainnet with real FXRP, real FTSOv2 feeds, real FDC attestations | After audit | 10+ active vault positions |
| **3. Pilot lending pool** | Onboard 3 institutional USDT0 lenders via adapter integration | Q1 2027 | $500K TVL |
| **4. AI credit scoring** | FCC handler ingests an off-chain AI credit-scoring model inside the TEE for automated, private eligibility decisions | Q1 2027 | 70%+ automated approval rate |

### Production features (Q2 2027+)

5. **Key governance** — TEE authority key rotation, revocation, multi-authority consensus (2-of-3 signers)
6. **ERC-3643 compliance** — Institutional compliance modules for regulated asset issuance, integrating permissioned FXRP transfers
7. **Multi-collateral** — FBTC, FDOGE credit gates with asset-specific risk parameters
8. **Adapter integration** — Gate access to existing lending markets (Morpho / Mystic) for institutional USDT0 supply

### Monetization / Sustainability

CreditGate sustains itself through two protocol-level mechanisms already coded in the vault:

- **5% APR interest** — Borrowers pay interest on drawn USDT0. Lenders receive the majority; a protocol slice funds operations.
- **1% protocol reserve** — 1% of interest-equivalent collateral accrues to a treasury (`protocolReserveBps`), owner-withdrawable to fund audits, TEE infrastructure, and emergency backstop.
- **Future: origination fee** — 10–25 bps on loan draw, configurable by governance.
...
*If the repo is set to private at judging time, contact via DoraHacks — it will be made public for the submission window. The repository contains the full Solidity vault, Foundry test suite (`forge test` → 195 tests / 20 suites / 0 failures), 33 Python TEE handler tests (`pytest`), FCC Go handler, Python TEE handler, and Next.js frontend.*
